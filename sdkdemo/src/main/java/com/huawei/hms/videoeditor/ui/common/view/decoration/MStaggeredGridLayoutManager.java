 ...
 */
public class MStaggeredGridLayoutManager extends RecyclerView.LayoutManager
     ...
     */
    private static final float MAX_SCROLL_FACTOR = BigDecimalUtils.div(1, 3f);
    ...
    boolean checkForGaps() {
        ...
        final LazySpanLookup.FullSpanItem validFsi =
            mLazySpanLookup.getFirstFullSpanItemInRange(minPos, invalidFsi.mPosition, invalidGapDir * -1, true);
        if (validFsi == null) {
        ...
        for (int i = firstChildIndex; i != childLimit; i += nextChildDiff) {
            ...
            if (i + nextChildDiff != childLimit) {
                ...
                if (compareSpans) {
                    ...
                    if (nextChild != null) {
                        if (nextChild.getLayoutParams() instanceof LayoutParams) {
    ...
    private void onLayoutChildren(RecyclerView.Recycler recycler, RecyclerView.State state,
        ...
        boolean recalculateAnchor =
            !anchorInfo.mValid || mPendingScrollPosition != RecyclerView.NO_POSITION || mPendingSavedState != null;
    ...
    private void applyPendingSavedState(AnchorInfo anchorInfo) {
        if (DEBUG) {
            SmartLog.d(TAG, "found saved state: " + mPendingSavedState);
        }
    ...
    void updateAnchorInfoForLayout(RecyclerView.State state, AnchorInfo anchorInfo) {
        ...
        if (DEBUG) {
            SmartLog.d(TAG, "Deciding anchor info from fresh state");
        }
    ...
    private boolean updateAnchorFromChildren(RecyclerView.State state, AnchorInfo anchorInfo) {
        ...
        anchorInfo.mPosition = mLastLayoutFromEnd ? findLastReferenceChildPosition(state.getItemCount())
        ...
        if (mPendingSavedState == null || mPendingSavedState.mAnchorPosition == RecyclerView.NO_POSITION
            ...
            if (child != null) {
                ...
                if (childSize > mPrimaryOrientation.getTotalSpace()) {
                    ...
                    anchorInfo.mOffset = anchorInfo.mLayoutFromEnd ? mPrimaryOrientation.getEndAfterPadding()
                        : mPrimaryOrientation.getStartAfterPadding();
                    return true;
                }

                final int startGap =
                    mPrimaryOrientation.getDecoratedStart(child) - mPrimaryOrientation.getStartAfterPadding();
                if (startGap < 0) {
    ...
    private int computeScrollOffset(RecyclerView.State state) {
        ...
        return ScrollbarHelper.computeScrollOffset(state, mPrimaryOrientation,
            ...
            findFirstVisibleItemClosestToEnd(!mSmoothScrollbarEnabled), this, mSmoothScrollbarEnabled,
    ...
    private void measureChildWithDecorationsAndMargin(View child, LayoutParams lp, boolean alreadyMeasured) {
        if (lp.mFullSpan) {
            if (mOrientation == VERTICAL) {
                measureChildWithDecorationsAndMargin(child, mFullSizeSpec, getChildMeasureSpec(getHeight(),
                    getHeightMode(), getPaddingTop() + getPaddingBottom(), lp.height, true), alreadyMeasured);
            } else {
                measureChildWithDecorationsAndMargin(child, getChildMeasureSpec(getWidth(), getWidthMode(),
                    getPaddingLeft() + getPaddingRight(), lp.width, true), mFullSizeSpec, alreadyMeasured);
            }
        } else {
            if (mOrientation == VERTICAL) {
                ...
                measureChildWithDecorationsAndMargin(child,
                    ...
                    getChildMeasureSpec(getHeight(), getHeightMode(), getPaddingTop() + getPaddingBottom(), lp.height,
                        true),
                    alreadyMeasured);
            } else {
                ...
                // factored into mSizePerSpan.
                measureChildWithDecorationsAndMargin(child,
                    getChildMeasureSpec(getWidth(), getWidthMode(), getPaddingLeft() + getPaddingRight(), lp.width,
                        true),
                    getChildMeasureSpec(mSizePerSpan, getHeightMode(), 0, lp.height, false), alreadyMeasured);
            }
        }
    }
    ...
    boolean shouldMeasureChild(View child, int widthSpec, int heightSpec, RecyclerView.LayoutParams lp) {
        return child.isLayoutRequested() || !isMeasurementUpToDate(child.getWidth(), widthSpec, lp.width)
    ...
    private int updateSpecWithExtra(int spec, int startInset, int endInset) {
        ...
        if (mode == View.MeasureSpec.AT_MOST || mode == View.MeasureSpec.EXACTLY) {
            return View.MeasureSpec.makeMeasureSpec(Math.max(0, View.MeasureSpec.getSize(spec) - startInset - endInset),
                mode);
        }
        return spec;
    }
    ...
    public void onRestoreInstanceState(Parcelable state) {
        if (state instanceof SavedState) {
            ...
            SmartLog.d(TAG, "invalid saved state class");
        }
    }
    ...
    public Parcelable onSaveInstanceState() {
        ...
        if (DEBUG) {
            SmartLog.d(TAG, "saved state:" + state);
        }
        return state;
    }
    ...
    public void onInitializeAccessibilityNodeInfoForItem(RecyclerView.Recycler recycler, RecyclerView.State state,
        ...
        if (mOrientation == HORIZONTAL) {
            info.setCollectionItemInfo(AccessibilityNodeInfoCompat.CollectionItemInfoCompat.obtain(sglp.getSpanIndex(),
                sglp.mFullSpan ? mSpanCount : 1, -1, -1, false, false));
        } else { // VERTICAL
    ...
    int findFirstVisibleItemPositionInt() {
        final View first =
            mShouldReverseLayout ? findFirstVisibleItemClosestToEnd(true) : findFirstVisibleItemClosestToStart(true);
    ...
    private void handleUpdate(int positionStart, int itemCountOrToPosition, int cmd) {
        ...
        switch (cmd) {
            ...
            case AdapterHelper.UpdateOp.MOVE:
                ...
                break;
            default:
                SmartLog.i(TAG, "handleUpdate run in default case");
        }
    ...
    private int fill(RecyclerView.Recycler recycler, LayoutState layoutState, RecyclerView.State state) {
        ...
        if (DEBUG) {
            SmartLog.d(TAG, "FILLING targetLine: " + targetLine + "," + "remaining spans:" + mRemainingSpans
                + ", state: " + layoutState);
        }
        ...
        while (layoutState.hasMore(state) && (mLayoutState.mInfinite || !mRemainingSpans.isEmpty())) {
            ...
            if (assignSpan) {
                ...
                if (DEBUG) {
                    SmartLog.d(TAG, "assigned " + currentSpan.mIndex + " for " + position);
                }
            } else {
                if (DEBUG) {
                    SmartLog.d(TAG, "using " + spanIndex + " for pos " + position);
                }
    ...
    private Span getNextSpan(LayoutState layoutState) {
        ...
        if (layoutState.mLayoutDirection == LayoutState.LAYOUT_END) {
            ...
            if (min != null) {
                if (mSpans[0].getEndLine(mPrimaryOrientation.getStartAfterPadding())
                    - Constant.STAGGERED_COL_HEIGHT_DIFF_VALUE > mSpans[1]
                        .getEndLine(mPrimaryOrientation.getStartAfterPadding())) {
                    return mSpans[1];
                } else {
                    return mSpans[0];
                }
    ...
    int scrollBy(int dt, RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        if (DEBUG) {
            SmartLog.d(TAG, "asked " + dt + " scrolled" + totalScroll);
        }
    ...
    int getLastChildPosition() {
        ...
        return childCount == 0 ? 0 : getPosition(currentView);
    }
    ...
    public View onFocusSearchFailed(View focused, int direction, RecyclerView.Recycler recycler,
        ...
        if (!prevFocusFullSpan) {
            unfocusableCandidate =
                findViewByPosition(shouldSearchFromStart ? prevFocusSpan.findFirstPartiallyVisibleItemPosition()
        ...
        if (preferLastSpan(layoutDir)) {
            for (int i = mSpanCount - 1; i >= 0; i--) {
                ...
                unfocusableCandidate =
                    findViewByPosition(shouldSearchFromStart ? mSpans[i].findFirstPartiallyVisibleItemPosition()
    ...
    private int convertFocusDirectionToLayoutDirection(int focusDirection) {
        switch (focusDirection) {
            ...
            default:
                if (DEBUG) {
                    SmartLog.d(TAG, "Unknown focus request:" + focusDirection);
                }
    ...
    class Span {
        ...
        public int findFirstVisibleItemPosition() {
            return mReverseLayout ? findOneVisibleChild(mViews.size() - 1, -1, false)
                : findOneVisibleChild(0, mViews.size(), false);
        }

        public int findFirstPartiallyVisibleItemPosition() {
            return mReverseLayout ? findOnePartiallyVisibleChild(mViews.size() - 1, -1, true)
                : findOnePartiallyVisibleChild(0, mViews.size(), true);
        }

        public int findFirstCompletelyVisibleItemPosition() {
            return mReverseLayout ? findOneVisibleChild(mViews.size() - 1, -1, true)
                : findOneVisibleChild(0, mViews.size(), true);
        }

        public int findLastVisibleItemPosition() {
            return mReverseLayout ? findOneVisibleChild(0, mViews.size(), false)
                : findOneVisibleChild(mViews.size() - 1, -1, false);
        }

        public int findLastPartiallyVisibleItemPosition() {
            return mReverseLayout ? findOnePartiallyVisibleChild(0, mViews.size(), true)
                : findOnePartiallyVisibleChild(mViews.size() - 1, -1, true);
        }

        public int findLastCompletelyVisibleItemPosition() {
            return mReverseLayout ? findOneVisibleChild(0, mViews.size(), true)
         ...
         */
        int findOnePartiallyOrCompletelyVisibleChild(int fromIndex, int toIndex, boolean completelyVisible,
            boolean acceptCompletelyVisible, boolean acceptEndPointInclusion) {
    ...
    static class LazySpanLookup {
        ...
        public FullSpanItem getFirstFullSpanItemInRange(int minPos, int maxPos, int gapDir,
            ...
            for (int i = 0; i < limit; i++) {
                ...
                if (fsi.mPosition >= minPos
                    && (gapDir == 0 || fsi.mGapDir == gapDir || (hasUnwantedGapAfter && fsi.mHasUnwantedGapAfter))) {
                    return fsi;
                }
            }
            return null;
        ...
        static class FullSpanItem implements Parcelable {
            ...
            public String toString() {
                return "FullSpanItem{" + "mPosition=" + mPosition + ", mGapDir=" + mGapDir + ", mHasUnwantedGapAfter="
                    + mHasUnwantedGapAfter + ", mGapPerSpan=" + Arrays.toString(mGapPerSpan) + '}';
            }
    ...
    class AnchorInfo {
        ...
        void assignCoordinateFromPadding() {
            mOffset =
                mLayoutFromEnd ? mPrimaryOrientation.getEndAfterPadding() : mPrimaryOrientation.getStartAfterPadding();
        }
