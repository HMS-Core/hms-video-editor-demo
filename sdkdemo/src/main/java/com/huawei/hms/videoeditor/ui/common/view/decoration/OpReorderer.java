...
class OpReorderer {
    private static final String TAG = "OpReorderer";
    ...
    void swapMoveRemove(List<AdapterHelper.UpdateOp> list, int movePos, AdapterHelper.UpdateOp moveOp, int removePos,
        ...
        if (moveOp.positionStart <= removeOp.positionStart) {
            ...
            extraRm =
                mCallback.obtainUpdateOp(AdapterHelper.UpdateOp.REMOVE, moveOp.positionStart + 1, remaining, null);
    ...
    void swapMoveUpdate(List<AdapterHelper.UpdateOp> list, int move, AdapterHelper.UpdateOp moveOp, int update,
        ...
        if (moveOp.itemCount < updateOp.positionStart) {
            ...
            updateOp.itemCount--;
            extraUp1 =
                mCallback.obtainUpdateOp(AdapterHelper.UpdateOp.UPDATE, moveOp.positionStart, 1, updateOp.payload);
        }
        // now affect of add is consumed. now apply effect of first remove
