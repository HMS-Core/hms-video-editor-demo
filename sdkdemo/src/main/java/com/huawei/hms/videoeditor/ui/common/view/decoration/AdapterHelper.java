...
class AdapterHelper implements OpReorderer.Callback {
    ...
    private void dispatchAndUpdateViewHolders(UpdateOp op) {
        ...
        if (DEBUG) {
            SmartLog.d(TAG, "dispatch (pre)" + op);
            SmartLog.d(TAG, "postponed state before:");
            for (UpdateOp updateOp : mPostponedList) {
                SmartLog.d(TAG, updateOp.toString());
            }
            SmartLog.d(TAG, "----");
        }
        ...
        if (DEBUG) {
            SmartLog.d(TAG, "pos:" + op.positionStart + ",updatedPos:" + tmpStart);
        }
        int tmpCnt = 1;
        ...
        for (int p = 1; p < op.itemCount; p++) {
            ...
            if (DEBUG) {
                SmartLog.d(TAG, "pos:" + pos + ",updatedPos:" + updatedPos);
            }
            ...
            switch (op.cmd) {
                ...
                case UpdateOp.REMOVE:
                    ...
                    break;
                default:
                    SmartLog.i(TAG, "dispatchAndUpdateViewHolders run in default case");
            }
            if (continuous) {
                tmpCnt++;
            } else {
                ...
                if (DEBUG) {
                    SmartLog.d(TAG, "need to dispatch separately " + tmp);
                }
        ...
        if (tmpCnt > 0) {
            ...
            if (DEBUG) {
                SmartLog.d(TAG, "dispatching:" + tmp);
            }
            dispatchFirstPassAndUpdateViewHolders(tmp, offsetPositionForPartial);
            recycleUpdateOp(tmp);
        }
        if (DEBUG) {
            SmartLog.d(TAG, "post dispatch");
            SmartLog.d(TAG, "postponed state after:");
            for (UpdateOp updateOp : mPostponedList) {
                SmartLog.d(TAG, updateOp.toString());
            }
            SmartLog.d(TAG, "----");
    ...
    private int updatePositionWithPostponed(int pos, int cmd) {
        ...
        for (int i = count - 1; i >= 0; i--) {
            ...
            if (DEBUG) {
                SmartLog.d(TAG, "dispath (step" + i + ")");
                SmartLog.d(TAG, "postponed state:" + i + ", pos:" + pos);
                for (UpdateOp updateOp : mPostponedList) {
                    SmartLog.d(TAG, updateOp.toString());
                }
                SmartLog.d(TAG, "----");
    ...
    static class UpdateOp {
        ...
        public String toString() {
            return Integer.toHexString(System.identityHashCode(this)) + "[" + cmdToString() + ",s:" + positionStart
                + "c:" + itemCount + ",p:" + payload + "]";
        }
